...
- If a subtask title or description contains Korean, Chinese, Japanese, emoji, or other non-ASCII text, do not pass inline JSON to `curl -d` or PowerShell native command arguments. Use `node scripts/paperclip-issue-create.mjs` or send a UTF-8 JSON file with `--data-binary` and `Content-Type: application/json; charset=utf-8`; otherwise Windows shells can turn the text into `????`. In PowerShell, write UTF-8 files with `Set-Content -Encoding utf8` and pass `--description-file` or `--payload-file`.
